...
export default function Communications() {
  const { triggerToast, user, token } = useOutletContext();
...
const messagesContainerRef = useRef(null);
  ...
  const fetchContacts = () => {
    ...
    fetch(`${API_BASE_URL}/messages/registrar-contacts`, { headers: getHeaders() })
      .then(r => r.json())
      .then(data => {
        if (data.success) {
          setContacts(data.contacts);
          setActiveContact(prev => prev || (data.contacts.length > 0 ? data.contacts[0] : null));
        }
      })
  ...
  useEffect(() => {
    ...
    setChatLoading(true);
    fetch(`${API_BASE_URL}/messages/conversation/${activeContact.role}/${activeContact.id}`, { headers: getHeaders() })
      .then(r => r.json())
      .then(data => {
  ...
  const contactLabel = (c) => {
    if (c.role === 'student') return c.subtitle || 'طالب';
    if (c.role === 'supervisor') return 'مشرف أكاديمي';
    return c.subtitle || '';
  };

  return (
    <div className="space-y-6 animate-fade-in text-right max-w-6xl mx-auto font-cairo select-none">
      <div className="border-b border-gray-100 pb-3">
        <h1 className="text-xl lg:text-2xl font-extrabold text-gray-800">مركز التواصل المباشر</h1>
        <p className="text-gray-400 text-xs font-semibold">مراسلة الطلاب والمشرفين الأكاديميين</p>
      </div>
        ...
        <div className="border-l border-gray-100 flex flex-col bg-gray-50/20">
          <div className="p-4 border-b border-gray-50 shrink-0 space-y-3">
            <h3 className="text-xs font-extrabold text-gray-800 flex items-center gap-1.5">
              <MessageSquare className="h-4.5 w-4.5 text-brand-purple" /> جهات الاتصال ({contacts.length})
            </h3>
          ...
          <div className="p-2 space-y-1.5 flex-1 overflow-y-auto min-h-[400px]">
            ...
            {!contactsLoading && filteredContacts.map(c => {
              ...
              const isActive = activeContact?.id === c.id && activeContact?.role === c.role;
              return (
                <div
                  key={`${c.role}-${c.id}`}
                ...
                >
                  <div className="text-right space-y-0.5 overflow-hidden flex-1">
                    ...
                    <p className="text-[10px] text-gray-400 truncate">{contactLabel(c)}</p>
                  </div>
        ...
        <div className="lg:col-span-2 flex flex-col justify-between bg-white h-[600px]">
          {!activeContact ? (
            <div className="flex-1 flex items-center justify-center text-gray-400 text-xs font-bold">
              {contactsLoading ? 'جاري تحميل جهات الاتصال...' : 'اختر جهة اتصال من القائمة لبدء المحادثة'}
            </div>
          ) : (
            <>
              {/* Chat Header */}
              ...
              {/* Messages Log */}
              <div ref={messagesContainerRef} className="flex-1 p-4 space-y-3 overflow-y-auto bg-gray-50/10">
                {chatLoading && (
                ...
                {!chatLoading && chatMessages.map((msg) => {
                  const isOwn = msg.sender_role === 'registrar';
                  return (
                    <div key={msg.message_id} className={`flex items-start gap-2 max-w-sm lg:max-w-md ${isOwn ? 'mr-auto flex-row-reverse' : 'ml-auto'}`}>
                      <div className={`h-7 w-7 rounded-full flex items-center justify-center font-bold text-[10px] shrink-0 ${
                        ...
                        {isOwn ? (user?.name?.charAt(0) || 'م') : activeContact.full_name?.charAt(0)}
                      </div>
