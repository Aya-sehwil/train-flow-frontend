...
import { SupervisorSocketProvider, useSupervisorSocket } from '../context/SupervisorSocketContext';

function SupervisorLayoutInner({ triggerToast, toast, user, token }) {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const socketCtx = useSupervisorSocket();
    ...
    <div className="flex min-h-screen w-full bg-[#f8f9fd] font-cairo select-none" dir="rtl">
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <div className="flex-1 p-4 lg:p-6 bg-[#f8f9fd]">
          <Outlet context={{ triggerToast, user, token, ...socketCtx }} />
        </div>
      </div>
    </div>
  );
...
export default function SupervisorLayout() {
  ...
  return (
    <SupervisorSocketProvider user={user} token={token} triggerToast={triggerToast}>
      <SupervisorLayoutInner triggerToast={triggerToast} toast={toast} user={user} token={token} />
    </SupervisorSocketProvider>
  );
}